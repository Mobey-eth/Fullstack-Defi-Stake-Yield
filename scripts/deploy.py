...
def deploy_token_farm_and_dapp_token(front_end_update=False):
    ...
    print(f"DappToken contract deployed at {dapp_token.address}")
    print(f"Token farm contract deployed at {token_farm.address}")
    # We need to send the token farm some dapp token so we can use it for rewards
    tx = dapp_token.transfer(
        token_farm.address, dapp_token.totalSupply() - Kept_balance, {"from": account}
    )
    tx.wait(1)

    # Platform would stake 3 tokens
